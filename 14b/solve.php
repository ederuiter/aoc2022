...
function extendGrid($grid)
{
    ...
    return [$grid, [count($grid[0]), count($grid)]];
}

function dropSand($grid, $source, $abyss)
{
    $pos = $source;
    while (true) {
        if ($pos[1] === $abyss) {
...
$rested = 0;
while (true) {
    $restPos = dropSand($grid, $sourceNormalized, $abyss);
    $grid[$restPos[1]][$restPos[0]] = 'O';
    $rested += 1;
    if ($restPos === $sourceNormalized) {
        break;
    }

    if ($restPos[0] === 0 || $restPos[0] === ($gridSize[0] - 1)) {
        [$grid, $gridSize] = extendGrid($grid);
        $sourceNormalized[0] += 1;
    }
}

printGrid($grid);
echo "Units of sand that came to rest before it stops the sand: $rested\n";
