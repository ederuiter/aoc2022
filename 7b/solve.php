...
$freeSpace = $capacity - $fs->getSize();
$toDelete = $requiredFreeSpace - $freeSpace;

echo "We need to delete at least: ", $toDelete, "\n";

$size = $fs->iterate(function ($carry, Item $item) use ($toDelete) {
    $size = $item->getSize();
    if ($item->getType() === 'dir' && $size >= $toDelete && $size < $carry) {
        echo " => ", $size, " ", $item->getRealPath(), "\n";
        $carry = $size;
    }
    return $carry;
}, $fs->getSize());
